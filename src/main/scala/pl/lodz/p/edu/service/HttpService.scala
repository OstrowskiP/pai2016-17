...
import pl.lodz.p.edu.dao.impl.{MongoDBUsersDao, MongoDBWindowsDao}
  ...
  def run() = {
    ...
    val task = new Runnable {
      def run() {
        ...
        val docList = usersDbHandler.extractDoc(users.value.get).toList
        ...
        val responseStatus = client.post(jsonArrStr, "post")
